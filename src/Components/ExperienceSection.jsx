...
//   return (
//     <section className="w-full bg-[#fce4ec] py-30  ">
//       <header 
//       data-aos="fade-down"
//         data-aos-offset="50"
//         data-aos-delay="100"
  ...
  return (
    <section className="w-full bg-[#fce4ec] py-30">
      <header 
        ...
        className="text-center mb-10"
      >
        <span className="font-['Poppins'] text-[10px] tracking-[0.6em] uppercase text-[#334155] font-normal opacity-60">
          My Experience
        </span>
        <h2 className="font-['Playfair_Display'] text-[#b4647d] text-5xl font-normal mt-2">Experience</h2>
      </header>
      ...
      >
            ...
            <div key={i} className="md:scale-[1.8] md:ml-44 origin-left">
              <Folder 
                items={[
                  <div className="p-4 h-full flex flex-col items-start justify-center">
                    {exp.role.map((line, idx) => (
                      <h4 key={idx} className="text-[8px] uppercase text-[#334155f0] font-normal leading-relaxed text-center">
                        {line}
                      </h4>
                    ))}
                  </div>,
                  <div className="p-4 h-full flex flex-col items-end justify-center">
                    {exp.company.map((line, idx) => (
                      <p key={idx} className="text-[9px] text-[#b4647d] leading-relaxed font-normal text-center">
                        {line}
                      </p>
                    ))}
                  </div>,
                  <div className="p-3 h-full flex items-center justify-center">
                    <span className="text-[6px] font-normal uppercase tracking-widest text-gray-400">{exp.year}</span>
                  </div>
                ]}
              />
            </div>
        ...
        <motion.a 
          href="/resume.pdf" 
          download="Aayusha_Ghimire_Resume.pdf"
          whileHover={{ scale: 1.05, backgroundColor: "#b4647d", color: "#ffffff" }}
          whileTap={{ scale: 0.97 }}
          className="flex items-center justify-center text-[8px] uppercase tracking-[0.2em] text-[#b4647d] border border-[#b4647d]/30 px-6 py-3 lg:ml-30 rounded-full bg-white/40 backdrop-blur-md transition-colors font-normal font-['Poppins'] cursor-pointer"
        >
          Download My Resume <span className="ml-2">→</span>
        </motion.a>
      </div>
